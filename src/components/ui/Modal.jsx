...
const Modal = ({ 
  ...
  return createPortal(
    <div 
      className={`fixed inset-0 z-50 flex ${alignEnd ? 'items-end justify-center' : 'items-center justify-center'} ${fullScreenOnMobile ? 'p-0 md:p-6' : (alignEnd ? 'p-0 sm:p-6' : 'p-4 sm:p-6')} bg-black/50 backdrop-blur-sm transition-opacity duration-300 ${visible ? 'opacity-100' : 'opacity-0'}`}
      onClick={onClose}
    >
      <div 
